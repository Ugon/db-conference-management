...
AS
	...
	(select top 1 Date from Day where Day.ConferenceID = Conference.ConferenceID order by Date ASC) as 'Data rozpoczęcia' 
...
AS
	select top 10 T.ClientId as 'Identyfikator', T.CompanyName as 'Nazwa klienta', T.TotalMoneySpent as 'Wydał łšcznie' from
...
AS
	...
	sum(Price) as 'Do zapłaty', 
	sum(Paid) as 'Zapłacono', 
